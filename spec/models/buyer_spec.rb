...
RSpec.describe Buyer, type: :model do
  ...
  describe '配送情報登録' do
    ...
    context '新規登録できない場合' do
      ...
      @buyer.house_number = ''
      @buyer.valid?
      expect(@buyer.errors.full_messages).to include("House_number code can't be blank")
      end 

      it '電話番号が空では保存できない' do
      @buyer.phone_number = ''
      @buyer.valid?
      expect(@buyer.errors.full_messages).to include("Phone_number code can't be blank")
      end 

      it '電話番号は、10桁以上11桁以内の半角数値でなければ保存できない' do
      @buyer.phone_number = '090-1234-56789'
      @buyer.valid?
      expect(@buyer.errors.full_messages).to include("Phone number is too short (minimum is 10 characters)")
      end 
    end
  end
end